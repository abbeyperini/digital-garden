...
I am floored only two of the meme alt-texts from [Object Oriented Programming in Memes](/blog.html?blog=OOP) were longer than 150 characters. Unfortunately, the polymorphism meme had "image," but I can see why I wanted to delineate where the caption ended and the image of the claymation pirate began.
...
At some point after this blog series and the Github issues, I'd like to come back and refactor my CSS to use far fewer margins. I'll be looking into `flex-basis` and might even switch to using CSS grid. I should also come up with a solution for my local lambda server that doesn't involve hardcoded links. (You can read about how it ended up that way in [the walkthrough](/blog.html?blog=serverless#:~:text=Then%20it%20was%20time%20to%20clean%20up%20my%20keys%20and%20routes.).) GitGuardian is saying I committed my DEV API key even though I tried really hard not to, so I revoked the one I was working with in this blog and replaced it when I deployed.
