...
Buttons are for performing an action (unless that action is navigating to another page). Buttons and inputs are in tab order and can be triggered with <kbd>space</kbd> and <kbd>enter</kbd> for keyboard users. The `<button>` element automatically provides hover and focus states.

Anchor tags (`<a>`) are for links and [provide useful SEO](https://www.semrush.com/blog/what-is-anchor-text-and-how-can-i-optimize-it/). Anchor tags are not in tab order and are only triggered on <kbd>enter</kbd>.
